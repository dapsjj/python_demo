    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if person_coordinate_center:#如果识别到人，则控制摄像头转动，把人放到屏幕中间
                ...
                #如果人刚好在屏幕中央，此时虽然不转动摄像头，但是也需要进行缩放
                elif max_area>0 and max_area <= (height * width) / proportion_of_the_largest_rectangle:  # 如果最大的人的矩形框的面积小于等于屏幕五分之一,就缩放
                    ZOOM_IN = mydll.VR_Control(lUserID, 1, 11, 0)  # 焦距变大
                    time.sleep(sleep_time)
                    ZOOM_IN = mydll.VR_Control(lUserID, 1, 11, 1)
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):  # q to quit
            ...
            mydll.VR_Clearup() #设备资源释放函数，无输入参数，成功返回 0，失败返回 <0 的错误码
            mydll.VR_Logout() #设备注销函数
            break
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='weights/20211020_face.pt', help='model.pt path(s)')
