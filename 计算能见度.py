...
def outputVisibilityValue(para_image_list):
    if para_image_list:
        ...
        for image_path in para_image_list:
            ...
            result_list.append([date, visibility_list[index],int(index)+1])

        with open(r'D:/pythonProjects/opencv_calculate_visibility/opencv_calculate_visibility.csv', 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerows(title)
            writer.writerows(result_list)

if __name__=='__main__':
    images_folder = r'C:/Users/Public/能见度爬虫/images'
